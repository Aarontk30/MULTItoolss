...
from config.util import Slow, Title, current_time_hour, Continue, Reset, Error, ErrorModule, ChoiceUserAgent, Censored
from config.config import scan_banner, BEFORE, AFTER, INPUT, WAIT, INFO, ADD, white, magenta, reset
...
def run():
    """ Fonction principale de l'outil Web Info. """
    ...
    try:
        ...
        def WebsiteFoundUrl(url):
            ...
            print(f"{BEFORE + current_time_hour() + AFTER} {ADD} Website: {white}{website_url}{magenta}")
            return website_url

        def WebsiteDomain(website_url):
            domain = urlparse(website_url).netloc
            print(f"{BEFORE + current_time_hour() + AFTER} {ADD} Domain: {white}{domain}{magenta}")
            return domain
        ...
        def WebsiteIp(domain):
            try:
                ...
                ip = "None"
            if ip != "None":
                print(f"{BEFORE + current_time_hour() + AFTER} {ADD} IP: {white}{ip}{magenta}")
            return ip
        ...
        def IpType(ip):
            if ':' in ip:
                ...
                return
            print(f"{BEFORE + current_time_hour() + AFTER} {ADD} IP Type: {white}{type}{magenta}")

        def WebsiteSecure(website_url):
            print(f"{BEFORE + current_time_hour() + AFTER} {ADD} Secure: {white}{website_url.startswith('https://')}{magenta}")
        ...
        def WebsiteStatus(website_url):
            try:
                ...
                status_code = 404
            print(f"{BEFORE + current_time_hour() + AFTER} {ADD} Status Code: {white}{status_code}{magenta}")
        ...
        def IpInfo(ip):
            ...
            for key in ['country', 'hostname', 'isp', 'org', 'asn']:
                if key in api:
                    print(f"{BEFORE + current_time_hour() + AFTER} {ADD} Host {key.capitalize()}: {white}{api[key]}{magenta}")
        ...
        def IpDns(ip):
            try:
                ...
                dns = "None"
            if dns != "None":
                print(f"{BEFORE + current_time_hour() + AFTER} {ADD} Host DNS: {white}{dns}{magenta}")
                ...
                try:
                    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
                        ...
                        if sock.connect_ex((ip, port)) == 0:
                            print(f"{BEFORE + current_time_hour() + AFTER} {ADD} Port: {white}{port}{magenta} Status: {white}Open{magenta} Protocol: {white}{port_protocol_map.get(port, 'Unknown')}{magenta}")
                except:
                    pass
        ...
        def HttpHeaders(website_url):
            try:
                ...
                for header, value in headers.items():
                    print(f"{BEFORE + current_time_hour() + AFTER} {ADD} HTTP Header: {white}{header}{magenta} Value: {white}{value}{magenta}")
            except RequestException:
                pass
        ...
        def CheckSslCertificate(website_url):
            try:
                ...
                for key, value in cert.items():
                    print(f"{BEFORE + current_time_hour() + AFTER} {ADD} SSL Certificate Key: {white}{key}{magenta} Value: {white}{value}{magenta}")
            except:
                pass
        ...
        def CheckSecurityHeaders(website_url):
            ...
            try:
                ...
                for header in headers:
                    print(f"{BEFORE + current_time_hour() + AFTER} {ADD} {'Missing' if header not in response_headers else 'Security'} Header: {white}{header}{magenta}")
            except RequestException:
                pass
        ...
        def AnalyzeCookies(website_url):
            try:
                ...
                for cookie in cookies:
                    ...
                    print(f"{BEFORE + current_time_hour() + AFTER} {ADD} Cookie: {white}{cookie.name}{magenta} Secure: {white}{secure}{magenta} HttpOnly: {white}{httponly}{magenta}")
            except RequestException:
                pass
        ...
        def DetectTechnologies(website_url):
            try:
                ...
                for tech in techs:
                    print(f"{BEFORE + current_time_hour() + AFTER} {ADD} Detected Technology: {white}{tech}{magenta}")
            except:
                pass
